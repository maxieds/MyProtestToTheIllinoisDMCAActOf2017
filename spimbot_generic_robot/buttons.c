...
/* $Header: /usr/local/cvsroot/spimbot/buttons.c,v 1.4 2004/02/04 04:08:53 craig Exp $
 */
...
typedef void (*WidgetAction) (_WidgetRec *, void *, void *);

static Widget breakpointButton;
static void (*confirmAction) (Widget w, XtPointer client_data, XtPointer call_data) = (WidgetAction) noop;
...
#endif
{
  ...
  if (run_popup == NULL)
    {
      sprintf (sa, "0x%08x", starting_address (reg_images[0]));
...
#endif
{
  ...
  addr = starting_address (reg_images[0]);
  init_stack (value2);
  if (steps > 0 && addr > 0) {
...
#endif
{
  ...
  destroy_popup_prompt (NULL, (XtPointer) dialog, (XtPointer) NULL);
  step_popup = NULL;
  execute_program (reg_images[0].PC, DEFAULT_RUN_STEPS, 0, 0);
}
...
#endif
{
  ...
  stack_initialized = 0;
  read_file (0, xspim_file_name, assem_or_exec);
}
...
#endif
{
  ...
  switch (clear_op)
    ...
    case CLEAR_REGS:
      ...
      initialize_registers (reg_images[0]);
      break;
  ...
  if (reg_no == 0)
  ...
  else
    {
      ...
      if (eptr == location_str)
		  ...
		  SET_MEM_WORD (/*context*/0, addr, value);
    }
  redisplay_data ();
}
...
#endif
{
  ...
  if (interrupt_seen)
    sprintf (msg, "execution interrupt at 0x%08x", reg_image.PC);
  else
    sprintf (msg, "breakpoint encountered at 0x%08x", reg_image.PC);
...
#endif
{
  ...
  destroy_popup_prompt (NULL, (XtPointer) dialog, (XtPointer) NULL);
  continue_popup = NULL;
  execute_program (reg_image.PC, 1, 0, 1); /* Step over breakpoint */
  execute_program (reg_image.PC, DEFAULT_RUN_STEPS - 1, 0, 0);
}
...
#endif
{
  ...
  confirmAction = (WidgetAction)noop;
...
#endif
{
  ...
  *field2_text = XtCreateManagedWidget ("field2_text", asciiTextWidgetClass,
  ...
  button1 = XtCreateManagedWidget (action_name, commandWidgetClass, form,
				   args, ONE);
  XtAddCallback (button1, XtNcallback, (WidgetAction)action, (XtPointer) form);
  ...
  if (action2 != NULL)
    {
      ...
      button2 = XtCreateManagedWidget (action2_name, commandWidgetClass, form,
				       args, TWO);
      XtAddCallback (button2, XtNcallback, (WidgetAction)action2, (XtPointer) form);
    }
